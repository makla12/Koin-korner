...
const getSeeds = async (gameType) => {
	let conn;
	try{
		conn = await pool.getConnection();
		const sql1 = '' +
		'SELECT server_seed.date as "date", server_seed.hashed_seed AS "serverSeed", public_seed.seed as "publicSeed", MAX(games.round) as "maxRound", MIN(games.round) as "minRound" FROM public_seed, server_seed, games '+
...
export { checkUsernameAndEmail, logIn, register, saveMessage, getMessages, getServerSeed, getPublicSeed, getGameRound, saveGameRound, getLast10Rolls, getBalance, saveBet, getLast10CrashScores, getSeeds};